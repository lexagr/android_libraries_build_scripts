...
$libPixmanBaseDir = "R:/gamedev/libraries/pixman"
$libFreetypeBaseDir = "R:/gamedev/libraries/freetype"
$libZlibBaseDir = "R:/gamedev/libraries/zlib"
...
[built-in options]
pkg_config_path = ['${libPixmanBaseDir}/build/dist/${archName}/lib/pkgconfig', '${libPngBaseDir}/build/${archName}', '${libFreetypeBaseDir}/build/${archName}', '${libZlibBaseDir}/build/${archName}/install/share/pkgconfig']
...
function Build-Library {
    ...
    & meson setup --cross-file="$CROSS_FILE" --prefix="$PREFIX" --buildtype=release `
        -Dglib=disabled -Dfreetype=enabled -Dfontconfig=disabled -Dzlib=enabled -Dtests=disabled `
